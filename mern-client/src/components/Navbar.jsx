    ...
    <header className='w-full bg-transparent fixed top-0 left-0 right-0 transition-all ease-in duration-300'>
      <nav className={`py-4 lg:px-24 px-4 ${isSticky?'sticky top-0 right-0 left-0 bg-blue-300':''}`}>
        <div className='flex justify-between items-center text-base gap-8'>
          ...
          <ul className=' md:flex space-x-12 hidden'>
            ...
            <div className='space-x-12 hidden lg:flex items-center'>
              <button type="button"><FaBarsStaggered className='w-5 hover:text-blue-700'/></button>
            </div>

            {/* menu btn form the mobile devices menu */}
        ...
        <div className={`space-y-4 px-4 mt-16 py-7 bg-sky-500 ${isMenuOpen?'block fixed top-0 right-0 left-0':'hidden'}`}>
          {
            navItems.map(({link,path})=> <Link key={path} to={path} className='block text-base text-white font-bold uppercase cursor-pointer'>{link}</Link>)
